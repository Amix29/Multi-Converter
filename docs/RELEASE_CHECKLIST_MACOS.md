...
- macOS `.app` bundles may contain internal symbolic links, especially inside frameworks. Packaging must preserve relative links and may normalize absolute links found inside `.framework` directories to same-framework relative links when the normalized target remains inside the same engine source tree. It may omit broken framework `Headers` or `PrivateHeaders` links, and it may preserve other same-tree framework symlinks for the framework layout to resolve after extraction, but it must still reject other absolute links, broken non-framework links and links that escape the engine source tree.
