...
- macOS `.app` bundles may contain internal symbolic links, especially inside frameworks. Packaging must preserve relative links and may normalize absolute links found inside `.framework` directories to same-framework relative links only when the normalized target exists inside the same engine source tree. It may omit broken framework `Headers` or `PrivateHeaders` links, but it must still reject other absolute links, broken runtime links and links that escape the engine source tree.
