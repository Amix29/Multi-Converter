...
- macOS `.app` bundles may contain internal symbolic links, especially inside frameworks. Packaging must preserve relative links and may normalize framework-rooted `/Versions/...` links to relative links, but it must still reject other absolute links, broken links and links that escape the engine source tree.
