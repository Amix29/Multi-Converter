...
Use raw executable files as inputs, not `.zip`, `.tar.gz`, `.tgz` archives or AppImages. Keep local input files outside the output folder because the output folder is cleaned first. This produces exactly `ffmpeg-x86_64-unknown-linux-gnu`, `ffprobe-x86_64-unknown-linux-gnu` and their `.sha256` files after checksum, placeholder, x86_64 ELF and `-version` checks. The manual `Linux Sidecar Staging` workflow performs the same validation on Ubuntu 22.04 and can optionally upload those four files to a test release.
...
Use the manual `Linux Sidecar Staging` workflow before `Linux AppImage Build` when the FFmpeg/ffprobe sidecars still need to be normalized into the expected release asset names. It requires explicit maintainer-approved binary URLs and SHA-256 values for both sidecars, rejects non-HTTPS URLs, rejects non-x86_64 ELF files and uploads a `linux-sidecar-assets` artifact. `Linux AppImage Build` rejects extra files in that artifact directory before staging the sidecars.
