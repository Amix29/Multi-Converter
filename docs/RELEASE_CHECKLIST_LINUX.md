...
LIBREOFFICE_LINUX_X64_DEB_ARCHIVE_SHA256="<sha256>" npm run prepare:libreoffice-engine:linux
PANDOC_LINUX_X64_ARCHIVE_SHA256="<sha256>" npm run prepare:pandoc-engine:linux
npm run prepare:libvips-engine:linux
```

These helpers must run on Linux x64. They prepare `engine-sources/linux-x64/pdfium`, `engine-sources/linux-x64/libreoffice`, `engine-sources/linux-x64/pandoc` and `engine-sources/linux-x64/libvips`, then the source-tree validator rechecks the staged outputs before packaging. The manual `Linux Engine Staging` workflow can run these helpers when `pdfium_upstream_sha256`, `libreoffice_upstream_sha256`, `pandoc_upstream_sha256` and `libvips_apt_runtime=true` are provided instead of the matching source-tree archive inputs. Source-tree archive inputs are mutually exclusive with their upstream/runtime preparation modes.
...
Use the manual `Linux Sidecar Staging` workflow before `Linux AppImage Build` when the FFmpeg/ffprobe sidecars still need to be normalized into the expected release asset names. It installs `tar`, `unzip` and `xz-utils`, requires explicit maintainer-approved executable or archive URLs and SHA-256 values for both sidecars, rejects non-HTTPS URLs, rejects AppImages, rejects non-x86_64 ELF files after extraction and uploads a `linux-sidecar-assets` artifact. Accepted archive formats are `.zip`, `.tar.gz`, `.tgz` and `.tar.xz`. `Linux AppImage Build` rejects extra files in that artifact directory before staging the sidecars.
