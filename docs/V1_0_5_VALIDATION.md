...
- Linux AppImage support is wired into configuration, release asset preparation/validation, README download links, updater metadata contracts and GitHub Actions. Local contract validation has covered Linux packaging and workflow tests, but the final AppImage build and full Linux conversion matrix still require a Linux host with the documented system packages and real Linux FFmpeg/ffprobe sidecars staged.
