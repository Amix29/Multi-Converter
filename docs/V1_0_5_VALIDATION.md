...
- Linux upstream PDFium/Pandoc preparation helpers: added on June 14, 2026. `npm run prepare:pdfium-engine:linux` and `npm run prepare:pandoc-engine:linux` now prepare reviewed Linux x64 source trees from SHA-256 pinned upstream PDFium/Pandoc archives and smoke-test them on Linux x64. This does not complete Linux advanced-engine staging because LibreOffice and libvips still require reviewed portable Linux runtime source trees.
- Linux engine staging upstream handoff: added on June 14, 2026. `Linux Engine Staging` can now prepare PDFium and Pandoc from pinned upstream SHA-256 inputs, revalidate those generated source trees with `npm run prepare:linux-engine-sources -- --allow-existing`, and package them together with required LibreOffice/libvips portable source-tree archives.
- `npm run check`: passed on June 14, 2026 after the Linux release-note, Linux ELF, Linux staged-engine manifest/source-tree, FFmpeg version-contract, workflow-trigger, Linux staging artifact handoff, Linux release-asset negative coverage, Linux local engine artifact strictness, Linux sidecar artifact strictness, Linux conversion manifest path hardening, Linux AppImage engine-set strictness, Linux AppImage metadata path hardening, Linux sidecar archive-source staging hardening, Linux `.tar.xz` staging compatibility, Linux `.tar.xz` extraction dependency hardening, Linux upstream PDFium/Pandoc preparation helper coverage and Linux engine staging upstream handoff.
