...
- `npm run check`: passed on June 14, 2026 after the Linux release-note, Linux ELF, Linux staged-engine manifest/source-tree, FFmpeg version-contract, workflow-trigger, Linux staging artifact handoff, Linux release-asset negative coverage, Linux local engine artifact strictness, Linux sidecar artifact strictness, Linux conversion manifest path hardening, Linux AppImage engine-set strictness, Linux AppImage metadata path hardening, Linux sidecar archive-source staging hardening, Linux `.tar.xz` staging compatibility, Linux `.tar.xz` extraction dependency hardening and Linux upstream PDFium/Pandoc preparation helper coverage.
