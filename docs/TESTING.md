...
The command writes `tmp/v1.0.5-status.json`. In the current preparation state, `releaseReady` should remain false because the clean-Mac Gatekeeper/install smoke test and the final security approval have not been recorded yet. When those final proofs exist, the same audit can report `releaseReady: true` instead of blocking that state.
