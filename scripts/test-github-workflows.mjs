...
assert.match(releasePreflightJob, /GitHub release notes must start with '# Multi-Converter \$tag'\./, "release preflight must validate the release note title");
assert.match(releasePreflightJob, /"## Highlights" "## Download And Installation" "## Validation"/, "release preflight must validate required release note sections");
assert.match(releasePreflightJob, /mc-release-notes:\(fr\|es\|de\|pt\|it\)/, "release preflight must reject localized release-note blocks");
assert.match(releasePreflightJob, /Release notes mention a macOS DMG, but this workflow run was not started with include_macos=true\./, "release preflight must reject accidental macOS notes in Windows-only runs");
