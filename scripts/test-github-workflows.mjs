...
assert.match(releasePreflightJob, /node scripts\/validate-release-notes\.mjs --version "\$version" --notes-file "\$notes_file" --include-macos "\$include_macos" --min-length 200/, "release preflight must use the shared release-note validator before allocating release runners");
