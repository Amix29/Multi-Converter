...
for (const sidecar of sidecars) {
  ...
  await assertRealLinuxSidecar(stagedSource, sidecar.id);

  const outputPath = path.join(outDir, sidecar.outputName);
  await fs.copyFile(stagedSource, outputPath);
...
function rejectArchiveSource(source) {
  const normalized = String(source).split(/[?#]/, 1)[0].toLowerCase();
  if (/\.(appimage|tar\.gz|tgz|zip)$/i.test(normalized)) {
    fail(`Linux sidecar source must be a raw ffmpeg/ffprobe executable, not an archive or AppImage: ${source}`);
  }
}
